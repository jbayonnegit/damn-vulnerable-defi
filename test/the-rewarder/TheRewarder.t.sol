	...
	function test_theRewarder() public checkSolvedByPlayer {
		...
		Claim[] memory		claimsDvt;
		{
			...
			Reward[] memory rewardsDvt =
				abi.decode(vm.parseJson(vm.readFile(string.concat(vm.projectRoot(), "/test/the-rewarder/dvt-distribution.json"))), (Reward[]));
			for ( uint256 i = 0; i < rewardsDvt.length; i++ )
			{
				if ( address(player) == rewardsDvt[i].beneficiary )
				{
					nodeDvt = i;
					amountDvt = rewardsDvt[i].amount;
					break ; 
				}
			}
			uint256	dvtRemain = distributor.getRemaining( address(tokensToClaim[0]) );
			uint256	maxDvtClaim = dvtRemain / amountDvt;
			bytes32[] memory	dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
			bytes32[] memory	_proofDvt = merkle.getProof( dvtLeaves, nodeDvt );
			claimsDvt = new Claim[]( maxDvtClaim );
			
			// Create Claim arrays
			for ( uint256 i = 0; i < maxDvtClaim; i++ )
			{
				claimsDvt[i] = Claim({
					batchNumber : 0,
					amount : amountDvt,
		...
		Claim[] memory		claimsWeth;
		{
			uint256	amountWeth;
			uint256 nodeWeth;
			address	beneficiarie;
			Reward[] memory rewardsWeth =
				abi.decode(vm.parseJson(vm.readFile(string.concat(vm.projectRoot(), "/test/the-rewarder/weth-distribution.json"))), (Reward[]));
			for ( uint256 i = 0; i < rewardsWeth.length; i++ )
			{
				if ( address(player) == rewardsWeth[i].beneficiary )
				{
					nodeWeth = i;
					beneficiarie = rewardsWeth[i].beneficiary;
					amountWeth = rewardsWeth[i].amount;
					break ; 
				}
			}
			console.log(amountWeth);
			console.log(nodeWeth);
			console.logAddress(address(player));
			console.logBytes32(keccak256(abi.encodePacked(address(player), amountWeth)));
			uint256	wethRemain = distributor.getRemaining( address(tokensToClaim[1]) );
			uint256	maxWethClaims = wethRemain / amountWeth;
			bytes32[] memory	wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
			bytes32[] memory	_proofWeth = merkle.getProof( wethLeaves, nodeWeth );
			claimsWeth = new Claim[]( maxWethClaims );
